...
import dev.blackhig.zhebushigudu.lover.features.gui.font.CustomFont;
...
public class TextManager extends Feature
{
    ...
    public void init(final boolean startup) {
        final FontMod cFont = lover.moduleManager.getModuleByClass(FontMod.class);
        try {
    ...
    public void drawString(final String text, final float x, final float y, final int color, final boolean shadow) {
        if (lover.moduleManager.isModuleEnabled(FontMod.getInstance().getName())) {
            if (shadow) {
