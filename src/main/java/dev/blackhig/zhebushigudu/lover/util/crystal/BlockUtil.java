...
public class BlockUtil
    ...
    static void processRightClickBlock(BlockPos pos, EnumFacing side, Vec3d hitVec) {
        BlockUtil.getPlayerController().processRightClickBlock(BlockUtil.mc.player, BlockUtil.mc.world, pos, side, hitVec, EnumHand.MAIN_HAND);
    }
    ...
    public static boolean placeBlockNotRetarded(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean altRotate) {
        ...
        if (rotate) {
            RotationUtil.faceVector(altRotate ? new Vec3d(pos) : hitVec, true);
        }
    ...
    public static void placeCrystalOnBlock(BlockPos pos, EnumHand hand) {
        BlockUtil.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(pos, EnumFacing.UP, hand, pos.getX(), pos.getY(), pos.getZ()));
    }
