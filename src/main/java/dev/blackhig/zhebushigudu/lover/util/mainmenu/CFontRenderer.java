...
public class CFontRenderer extends CFont
{
    ...
    public CFontRenderer(final CustomFont font, final boolean antiAlias, final boolean fractionalMetrics) {
        super(font, antiAlias, fractionalMetrics);
        this.boldChars = new CharData[256];
        this.italicChars = new CharData[256];
        this.boldItalicChars = new CharData[256];
        ...
        if (this.getStringWidth(text) > width) {
            ...
            for (final String word : words) {
                for (int i = 0; i < word.toCharArray().length; ++i) {
                    ...
                    if (c == '\u6402' && i < word.toCharArray().length - 1) {
                ...
                if (this.getStringWidth(stringBuilder.append(currentWord).append(word).append(" ").toString()) < width) {
                    ...
                    currentWord = "\u6402" + lastColorCode + word + " ";
                }
            }
            if (currentWord.length() > 0) {
                if (this.getStringWidth(currentWord) < width) {
                    finalWords.add("\u6402" + lastColorCode + currentWord + " ");
                    currentWord = "";
                }
                else {
