...
public class RenderUtils
{
    ...
    public static Void drawTexture(final ResourceLocation location, final int x, final int y, final int textureX, final int textureY, final int width, final int height) {
        RenderUtils.mc.getTextureManager().bindTexture(location);
        ...
        bufferbuilder.pos((double)(x + 0), (double)(y + height), 0.0).tex((double)((textureX + 0) * 0.00390625f), (double)((textureY + height) * 0.00390625f)).endVertex();
        bufferbuilder.pos((double)(x + width), (double)(y + height), 0.0).tex((double)((textureX + width) * 0.00390625f), (double)((textureY + height) * 0.00390625f)).endVertex();
        bufferbuilder.pos((double)(x + width), (double)(y + 0), 0.0).tex((double)((textureX + width) * 0.00390625f), (double)((textureY + 0) * 0.00390625f)).endVertex();
        bufferbuilder.pos((double)(x + 0), (double)(y + 0), 0.0).tex((double)((textureX + 0) * 0.00390625f), (double)((textureY + 0) * 0.00390625f)).endVertex();
        tessellator.draw();
        return null;
    }
    
    public static Void drawTexture2(final InputStream inputStream, final int x, final int y, final int textureX, final int textureY, final int width, final int height) {
        GlStateManager.bindTexture(loadTexture(inputStream).getId());
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        final float f = 0.00390625f;
        final float f2 = 0.00390625f;
        final Tessellator tessellator = Tessellator.getInstance();
    ...
    private void drawShadow(final Entity entity, final float partialTicks, final float pos, final boolean direction) {
        ...
        final double x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * partialTicks - RenderUtils.mc.getRenderManager().viewerPosX;
        final double y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * partialTicks - RenderUtils.mc.getRenderManager().viewerPosY + pos;
        final double z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * partialTicks - RenderUtils.mc.getRenderManager().viewerPosZ;
        GL11.glBegin(8);
        for (int i = 0; i <= 180; ++i) {
