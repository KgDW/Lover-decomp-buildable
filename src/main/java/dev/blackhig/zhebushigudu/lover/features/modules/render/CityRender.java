...
public class CityRender extends Module
{
    ...
    public CityRender() {
        super("CityRender", "CityRender", Category.RENDER, true, false, false);
    ...
    private void surroundRender(final Vec3d pos, final double x, final double y, final double z, final boolean red) {
        ...
        if (CityRender.mc.world.getBlockState(position).getBlock() == Blocks.AIR || CityRender.mc.world.getBlockState(position).getBlock() == Blocks.FIRE || CityRender.mc.world.getBlockState(position).getBlock() != Blocks.OBSIDIAN) {
            return;
        }
        if (red) {
    ...
    private EntityPlayer getTarget(final double range) {
        ...
        for (final EntityPlayer player : CityRender.mc.world.playerEntities) {
