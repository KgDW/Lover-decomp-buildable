...
public class AutoCev1 extends Module
{
    ...
    public AutoCev1() {
        ...
        Setting<Boolean> autoToggle = this.register(new Setting<>("AutoToggle", true));
        this.progress = 0;
    }
    ...
    public void onTick() {
        ...
        if (n != -1 && n2 != -1 && n3 != -1) {
            if (this.currentEntity == null || this.currentEntity.getDistance(AutoCev1.mc.player) > this.range.getValue()) {
                this.findTarget();
            }
                            ...
                            case 2: {
                                ...
                                for (final Entity entity3 : AutoCev1.mc.world.loadedEntityList) {
                                    if (entity.getDistance(entity3) <= this.range.getValue() && entity3 instanceof EntityEnderCrystal) {
                                        AutoCev1.mc.playerController.attackEntity(AutoCev1.mc.player, entity3);
                                        ++n5;
                                    }
                                }
                                if (n5 == 0 || this.flag) {
    ...
    public void findTarget() {
        this.currentEntity = AutoCev1.mc.world.loadedEntityList.stream().filter(entity -> entity != AutoCev1.mc.player && entity instanceof EntityLivingBase && entity.getDistance(AutoCev1.mc.player) < this.range.getValue() && !lover.friendManager.isFriend(entity.getName())).findFirst().orElse(null);
    }
}
