...
public class Skeleton extends Module
{
    ...
    private void drawSkeleton(final Render3DEvent event, final EntityPlayer e) {
        ...
        if (entPos != null && e.isEntityAlive() && !e.isDead && e != Skeleton.mc.player && !e.isPlayerSleeping()) {
            ...
            if (lover.friendManager.isFriend(e.getName())) {
                GlStateManager.color(0.0f, 191.0f, 230.0f, this.alpha.getValue());
            }
            else {
                GlStateManager.color(ClickGui.getInstance().red.getValue() / 255.0f, ClickGui.getInstance().green.getValue() / 255.0f, ClickGui.getInstance().blue.getValue() / 255.0f, this.alpha.getValue());
            }
