...
public class CityRenderPlus extends Module
{
    private final Setting<Integer> range;
    public EntityPlayer target;
    
    public CityRenderPlus() {
        super("City Y Render", "CityRender+", Category.RENDER, true, false, false);
    ...
    private void surroundRender() {
        if (this.target != null) {
            ...
            if (CityRenderPlus.mc.world.getBlockState(new BlockPos(a)).getBlock() == Blocks.OBSIDIAN || CityRenderPlus.mc.world.getBlockState(new BlockPos(a)).getBlock() == Blocks.ENDER_CHEST) {
    ...
    private void surroundRender(final Vec3d pos, final double x, final double y, final double z, final boolean red) {
        ...
        if (CityRenderPlus.mc.world.getBlockState(position).getBlock() == Blocks.AIR || CityRenderPlus.mc.world.getBlockState(position).getBlock() == Blocks.FIRE || CityRenderPlus.mc.world.getBlockState(position).getBlock() != Blocks.OBSIDIAN) {
            return;
        }
        if (red) {
            ...
            RenderUtil.drawBoxESP(position, new Color(0, 21, 255), false, new Color(0, 21, 255), 1.0f, false, true, 42, true);
        }
    }
    ...
    private EntityPlayer getTarget(final double range) {
        ...
        for (final EntityPlayer player : CityRenderPlus.mc.world.playerEntities) {
